...
try
    result = solve_eigenvalue_problem(A, B; nev=10, maxiter=200)

    println("✓ Eigenvalue problem solved successfully!")
    println()
    println("Leading eigenvalue:")
    println("  Growth rate (σ_r):      $(real(result.σ))")
    println("  Drift frequency (ω):    $(imag(result.σ))")
    println()

    if real(result.σ) > 0
        println("  → System is UNSTABLE (growing mode)")
        println("    Dynamo instability detected!")
    elseif real(result.σ) < 0
    ...
    println("  Iterations: $(result.numiter)")
    println("  Residual:   $(result.residual)")
    println()

    # Display top 5 eigenvalues
    if length(result.values) >= 5
        println("Top 5 eigenvalues:")
        for (i, λ) in enumerate(result.values[1:5])
