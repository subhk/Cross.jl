...
@testset "biglobal COO == radial + dense basic-state" begin
    ...
    coo = Cross._assemble_onset_coo(op)
    ...
    coo_radial = Cross._assemble_onset_radial_coo(op)
    ...
    bs_ops = Cross.build_basic_state_operators(op.params.basic_state, op, op.params.m)
    Cross.add_basic_state_operators!(A_ref, B_ref, bs_ops, op, op.params.m)
...
@testset "constraint sub-blocks match BC-overwritten A rows" begin
    ...
    for ℓ in op.l_sets[:P]
        idx = op.index_map[(ℓ,:P)]; ri,it,ot,ro = Cross.poloidal_tau_indices(idx)
        @test Cross._constraint_subblock(op, ℓ, :P) ≈ A_bc[[ri,it,ot,ro], idx] rtol=1e-10
    end
...
@testset "_onset_interior_dofs matches assemble_matrices" begin
    ...
    @test Cross._onset_interior_dofs(op) == idofs
end
