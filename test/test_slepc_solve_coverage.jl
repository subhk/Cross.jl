...
# (CROSS_SLEPC_TEST=1). Drives every high-level solve() dispatch through the SLEPc
...
if !(haskey(ENV, "PETSC_DIR") && get(ENV, "CROSS_SLEPC_TEST", "") == "1")
    @info "SLEPc real-solve coverage skipped (needs PETSC_DIR + CROSS_SLEPC_TEST=1 + PetscWrap/SlepcWrap)"
    @test true
else
